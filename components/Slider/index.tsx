...
const Slider: React.FC<PropsWithChildren> = ({children}) => {
    ...
    const arrowClass = "w-12 h-12 self-center cursor-pointer max-lg:hidden";
    ...
    return (
        <div className="grid grid-cols-[1fr_2fr_2fr_2fr_1fr] justify-items-center scrollbar mt-5 gap-5 max-lg:grid-cols-1 max-lg:px-5">
            {isMore ?
            ...
            {childrenArray.map((child, i)=>(<div key={i} className={`${currentRange.start <= i && i < currentRange.end ? "block" : "hidden"} w-full max-lg:block`}>{child}</div>))}
            {isMore ?
